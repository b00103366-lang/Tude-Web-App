...
export function Login() {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<z.infer<typeof schema>>({
    ...
    defaultValues: { email: "student@etude.tn", password: "password" }
  });
  ...
  const onSubmit = async (data: z.infer<typeof schema>) => {
    ...
    try {
      await loginFn(data);
      setLocation("/student/dashboard"); // Assuming auth wrapper handles correct redirect based on role
    } catch (e: any) {
      toast({ title: "Erreur", description: e.message || "Identifiants invalides", variant: "destructive" });
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-secondary/30 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <FadeIn className="w-full max-w-md">
        ...
        <Card className="p-8 shadow-xl">
          ...
          <div className="mt-6 bg-muted/50 rounded-lg p-4 text-xs text-muted-foreground text-center">
            <p className="font-semibold mb-1">Comptes de démo :</p>
            <p>Admin: admin@etude.tn</p>
            <p>Prof: prof@etude.tn</p>
            <p>Élève: student@etude.tn</p>
            <p className="mt-1">MDP: password</p>
          </div>
        </Card>
      </FadeIn>
    </div>
