/**
 * BanqueDeQuestionsSubject — chapter list for a subject.
 *
 * Fetches chapters from /api/curriculum/chapters (includes question/flashcard counts).
 * Chapters always render even when count = 0 — the curriculum drives the UI,
 * not the content tables.
 *
 * Clicking a chapter navigates to BanqueDeQuestionsTopic for the actual Q&A session.
 */
  ...
  return (
    <DashboardLayout>
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        {/* Empty — no chapters in curriculum yet */}
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {chapters.map((chapter, i) => (
              <ChapterCard
                ...
                chapter={chapter}
                index={i}
                href={`/revision/${slug}/banque-de-questions/${encodeURIComponent(chapter.name)}`}
              />
            ))}
          </div>
        )}
  ...
  return (
    <Link href={href}>
      <div className={cn(
        ...
        {/* Row: number + question count badge */}
