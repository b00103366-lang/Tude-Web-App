...
function QuestionTab() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const data = await apiFetch<{ id: number }>(`${API}/api/kb/questions/manual`, {
        method: "POST",
  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      {lastId && <SuccessBanner label="Question" id={lastId} />}
      ...
      <button type="submit" disabled={!subject || !gradeKey || !topic || !questionText || saving}
        ...
        {saving ? <><Loader2 className="w-4 h-4 animate-spin" /> Enregistrement...</> : <><CheckCircle2 className="w-4 h-4" /> Publier dans la banque de questions</>}
      </button>
    </form>
  );
}
