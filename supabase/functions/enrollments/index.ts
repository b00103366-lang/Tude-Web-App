...
Deno.serve(async (req: Request) => {
  ...
  const enrollIdx = parts.indexOf("enrollments");
  const sub = enrollIdx >= 0 ? parts.slice(enrollIdx + 1) : parts;
  const method = req.method;

  // GET /enrollments/my
  if (method === "GET" && sub[0] === "my") {
    return json({ enrollments: [] }, 200, req);
  }

  // POST /enrollments
  if (method === "POST") {
    return json({ id: null, message: "Enrollment not available" }, 201, req);
  }
  ...
  if (method === "GET") {
    try {
      ...
      if (!dbUrl) return json({ enrollments: [] }, 200, req);
      const sql = neon(dbUrl);
      const rows = await sql`
        SELECT * FROM enrollments ORDER BY created_at DESC LIMIT 100
      ` as Record<string, unknown>[];
      return json({ enrollments: rows.map(camel) }, 200, req);
    } catch {
      return json({ enrollments: [] }, 200, req);
    }
  }
